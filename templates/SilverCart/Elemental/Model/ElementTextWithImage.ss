<div class="row d-flex element-text-with-image" id="element-text-with-image-{$ID}">
    <div class="<% if $Ratio == '1:1' %>col-lg-6<% else %>col-lg-8<% end_if %> px-25 px-lg-45 py-30 py-lg-60 border-bottom border-lg-bottom-0 <% if $Layout == 'ContentRight' %>order-lg-2<% end_if %>" {$StyleText}>
        <% if $ShowTitle %><h2 class="text-uppercase mb-10 line-height-30px">{$Title.RAW}</h2><% end_if %>
        <% if $ShowSubTitle %><h3 class="text-uppercase mt-20 mb-10 line-height-22px">{$SubTitle.RAW}</h3><% end_if %>
        <% if $Content %><div class="text-lg text-justify">{$Content}</div><% end_if %>
    </div>
    <div class="<% if $Ratio == '1:1' %>col-lg-6<% else %>col-lg-4<% end_if %> px-5px px-sm-10 px-lg-20 px-xxl-30 py-5px py-sm-10 py-lg-20 py-xxl-30 border-bottom border-lg-bottom-0 <% if $Layout == 'ContentRight' %>order-lg-1<% end_if %>" {$StyleImage}>
